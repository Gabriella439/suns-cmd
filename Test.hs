...
tests = each parameters >-> Pipes.mapM test
  where
    test (relPath, rmsd, expectation) = do
        putStrLn $ "Testing: " ++ relPath
        bs <- expect ("test/" ++ relPath) rmsd
        putStrLn $ "Expected: " ++ show expectation
        putStrLn $ "Found   : " ++ show bs
        return (bs == expectation)
